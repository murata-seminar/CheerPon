...
class ViewController: UIViewController, CLLocationManagerDelegate {
    ...
    //メッセージデータをfirestoreに流し込む用の関数
    private func uploadMessageDataToFirestore(){
        //documentの名前：いずれ自動で作成
        var documentname = "aori_MidNight"
        for i in 0...(mnm.aori_MidNight.count - 1) {
            var message = mnm.aori_MidNight[i]
            db.collection("dialogue").document(documentname + i.description).setData([
                "line": message,
                "attitude": "aori_MidNight"
            ]) { err in
                if let err = err {
                    print("Firestoreのエラー Error adding document: \(err)")
                }else{
