            ...
            if mtcc.timer_counter <= 1800 {
                ...
                if mtcc.timer_counter % 900 == 0 {
                    ...
                    if mtcc.checkTime(from: 5, to: 11) {    //午前
                        message = mnm.getCommnet(comments: mnm.cheerpon_Counter_Morning)
                    }else if mtcc.checkTime(from: 11, to: 13){  //お昼
                        message = mnm.getCommnet(comments: mnm.cheerpon_Counter_Noon)
                    }else if mtcc.checkTime(from: 13, to: 18){  //午後
                        message = mnm.getCommnet(comments: mnm.cheerpon_Counter_Afternoon)
                    }else if mtcc.checkTime(from: 18, to: 23){  //夜
                        message = mnm.getCommnet(comments: mnm.cheerpon_Counter_Night)
                    }else{  //深夜（上記以外）
                        message = mnm.getCommnet(comments: mnm.cheerpon_Counter_MidNight)
                    }
            ...
            if mtcc.timer_counter == 2 {
                ...
                if self.mtcc.unlockedcounter != 0 && self.mtcc.unlockedcounter % 10 == 0 && self.mtcc.unlockedcounter <= 50 {
                    ...
                    if self.mtcc.checkTime(from: 5, to: 11) {    //午前
                        message = self.mnm.getCommnet(comments: self.mnm.cheerpon_Counter_Morning)
                    }else if self.mtcc.checkTime(from: 11, to: 13){  //お昼
                        message = self.mnm.getCommnet(comments: self.mnm.cheerpon_Counter_Noon)
                    }else if self.mtcc.checkTime(from: 13, to: 18){  //午後
                        message = self.mnm.getCommnet(comments: self.mnm.cheerpon_Counter_Afternoon)
