...
class ViewController: UIViewController, CLLocationManagerDelegate {
    ...
    @objc func update_lighton() {
        ...
        //他ユーザ
        let countNumOther = Int(total_time - 1280)
        let so2 = countNumOther % 60
        let mo2 = (countNumOther % 3600) / 60
        let ho2 =  countNumOther / 3600
        let displayStrOther = NSString(format: "%02d時間%02d分%02d秒", ho2,mo2,so2 ) as String
        TimerLabel2.text = displayStrOther
        TimerLabel3.text = String(countNum02 - countNumOther)
        
        let oncounterOther = mtcc.oncounter + 20
        CounterLabel2.text = String(oncounterOther)
        CounterLabel3.text = String(oncounterOther - mtcc.oncounter)
        
        //グラフ
        //割合を求める
        if(countNum02 >= countNumOther){
            let portion = Double(countNumOther) / Double(countNum02)
            //LabelA.frame = CGRect(x: 0, y: 0, width: 300, height: 300)
